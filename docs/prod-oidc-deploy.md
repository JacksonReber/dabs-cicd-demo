...
> additional CI target gets its own service principal and OIDC federation policy — scoped to
> `repo:<org>/<repo>:environment:<name>` — plus its own host/client-id repo variables.
> Everything below applies unchanged; just swap the environment name and SP. See the README
> "Caveats and decisions" section for the full set of changes to add a staging stage.
